...
fn strip_drm_metadata(xml: &[u8]) -> Vec<u8> {
    ...
    // Rust regex does not support look-around, so this is intentionally done
    // with a small XML-ish scanner instead of a lookahead regex.
    let mut cleaned = String::with_capacity(s.len());
    let mut rest = s.as_str();

    loop {
        let Some(start) = rest.find("<ContentProtection") else {
        ...
        let (block, tail) = if open_tag.trim_end().ends_with("/>") {
            (open_tag, &after[tag_end..])
        } else if let Some(close_rel) = after[tag_end..].find("</ContentProtection>") {
            let close_end = tag_end + close_rel + "</ContentProtection>".len();
            (&after[..close_end], &after[close_end..])
        } else {
            cleaned.push_str(after);
            break;
        };
